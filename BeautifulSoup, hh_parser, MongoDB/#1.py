...
links = vacancies.find({}, {'link': True, '_id': False})  # список словарей с ссылками на вакансию
links = set(x['link'] for x in links)  # преобразование к множеству (из-за скорости поиска)

for vac in vac_list:
    if vac['link'] not in links:
        vacancies.insert_one({'name': vac['name'],
                              'min_salary': vac['salary']['min_salary'],
                              'max_salary': vac['salary']['max_salary'],
                              'valuta': vac['salary']['valuta'],
                              'link': vac['link'],
                              'homepage': vac['homepage']})
